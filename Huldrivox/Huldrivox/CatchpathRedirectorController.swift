...
class CatchpathRedirectorController: UIViewController {
    private let scrollView = UIScrollView()
    private let contentView = UIView()
    
    private let titleLabel: UILabel = {
        let label = UILabel()
        label.text = "Catchpath Redirector Splitbeam Subshift Modulator Calculator"
        label.font = UIFont.systemFont(ofSize: 20, weight: .bold)
        label.textColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 1)
    ...
    private let inputCard1: UIView = {
        let view = UIView()
        view.backgroundColor = UIColor(red: 0.25, green: 0.2, blue: 0.2, alpha: 1)
        view.layer.cornerRadius = 12
        view.layer.borderWidth = 1
        view.layer.borderColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 0.5).cgColor
        return view
    }()
    
    private let inputField1: UITextField = {
        let field = UITextField()
        field.placeholder = "Enter Splitbeam Frequency"
        field.textColor = .white
        field.font = UIFont.systemFont(ofSize: 16)
        field.attributedPlaceholder = NSAttributedString(
            string: "Enter Splitbeam Frequency",
    ...
    private let inputField2: UITextField = {
        let field = UITextField()
        field.placeholder = "Enter Subshift Ratio"
        field.textColor = .white
        field.font = UIFont.systemFont(ofSize: 16)
        field.attributedPlaceholder = NSAttributedString(
            string: "Enter Subshift Ratio",
            attributes: [.foregroundColor: UIColor.lightGray]
        )
        field.keyboardType = .decimalPad
        return field
    ...
    private let inputField3: UITextField = {
        let field = UITextField()
        field.placeholder = "Enter Modulation Factor"
        field.textColor = .white
        field.font = UIFont.systemFont(ofSize: 16)
        field.attributedPlaceholder = NSAttributedString(
            string: "Enter Modulation Factor",
            attributes: [.foregroundColor: UIColor.lightGray]
        )
        field.keyboardType = .decimalPad
        return field
    ...
    private let segmentControl: UISegmentedControl = {
        let segment = UISegmentedControl(items: ["Standard", "Enhanced", "Quantum"])
        segment.selectedSegmentTintColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 1)
    ...
    private let switchLabel: UILabel = {
        let label = UILabel()
        label.text = "Enable Redirector Boost"
    ...
    private let boostSwitch: UISwitch = {
        let sw = UISwitch()
        sw.onTintColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 1)
        return sw
    }()
    ...
    private let menuButton: UIButton = {
        let button = UIButton()
        button.setTitle("Select Beam Type ▼", for: .normal)
        button.setTitleColor(UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 1), for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        return button
    }()
    
    private var menuSelection = "Single"
    ...
    private let chartView: UIView = {
        let view = UIView()
        view.backgroundColor = UIColor(red: 0.2, green: 0.15, blue: 0.15, alpha: 1)
        view.layer.cornerRadius = 12
        view.layer.borderWidth = 1
        view.layer.borderColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 0.5).cgColor
        view.alpha = 0
    ...
    private let calculateButton: UIButton = {
        ...
        button.backgroundColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 1)
        button.setTitleColor(.white, for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 18, weight: .bold)
        button.layer.cornerRadius = 25
        button.layer.shadowColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 0.5).cgColor
    ...
    private func setupUI() {
        ...
        gradientLayer.colors = [
            UIColor(red: 0.15, green: 0.1, blue: 0.1, alpha: 1).cgColor,
            UIColor(red: 0.25, green: 0.15, blue: 0.15, alpha: 1).cgColor,
            UIColor(red: 0.1, green: 0.05, blue: 0.05, alpha: 1).cgColor
        ]
    ...
    private func setupMenu() {
        let single = UIAction(title: "Single Beam", image: UIImage(systemName: "scope")) { _ in
            self.menuSelection = "Single"
            self.menuButton.setTitle("Single Beam ▼", for: .normal)
        }
        
        let dual = UIAction(title: "Dual Beam", image: UIImage(systemName: "scope.fill")) { _ in
            self.menuSelection = "Dual"
            self.menuButton.setTitle("Dual Beam ▼", for: .normal)
        }
        
        let quad = UIAction(title: "Quad Beam", image: UIImage(systemName: "square.split.2x2.fill")) { _ in
            self.menuSelection = "Quad"
            self.menuButton.setTitle("Quad Beam ▼", for: .normal)
        }
        
        let menu = UIMenu(title: "Select Beam Type", children: [single, dual, quad])
    ...
    private func setupParticles() {
        for _ in 0..<20 {
            ...
            particle.backgroundColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 0.6)
            particle.layer.cornerRadius = size / 2
            view.addSubview(particle)
            view.sendSubviewToBack(particle)
    ...
    private func animateParticle(_ particle: UIView) {
        let duration = Double.random(in: 5...15)
        let xMove = CGFloat.random(in: -50...50)
        let yMove = CGFloat.random(in: -50...50)
        
        UIView.animate(withDuration: duration, delay: 0, options: [.autoreverse, .repeat, .curveEaseInOut], animations: {
            particle.frame.origin.x += xMove
            particle.frame.origin.y += yMove
    ...
    @objc private func calculateTapped() {
        ...
        if menuSelection == "Dual" {
            result *= 1.5
        } else if menuSelection == "Quad" {
            result *= 2.0
        }
        ...
        resultLabel.text = """
        Catchpath Redirector Splitbeam Subshift Modulator Result:
        
        Frequency: \(value1)
        Ratio: \(value2)
        Factor: \(value3)
        Beam Type: \(menuSelection)
        Calculation Mode: \(segmentControl.titleForSegment(at: segmentControl.selectedSegmentIndex) ?? "")
        Boost: \(boostSwitch.isOn ? "Enabled" : "Disabled")
        
        Final Modulation: \(formattedResult)
        """
        ...
        drawRadarChart(result: result)
        playSound()
    }
    
    private func drawRadarChart(result: Double) {
        ...
        let maxRadius = min(chartView.bounds.width, chartView.bounds.height) * 0.4
        let normalizedResult = min(result / 100, 1.0)
        let actualRadius = maxRadius * CGFloat(normalizedResult)
        ...
        for i in 0..<points {
            ...
            let radius = i % 2 == 0 ? actualRadius : actualRadius * 0.7
        ...
        shapeLayer.fillColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 0.3).cgColor
        shapeLayer.strokeColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 1).cgColor
        shapeLayer.lineWidth = 2
        
        let animation = CABasicAnimation(keyPath: "path")
        animation.fromValue = UIBezierPath(ovalIn: CGRect(x: center.x, y: center.y, width: 0, height: 0)).cgPath
        animation.toValue = path.cgPath
        animation.duration = 1.0
        animation.timingFunction = CAMediaTimingFunction(name: .easeInEaseOut)
        
        shapeLayer.add(animation, forKey: "radarAnimation")
        chartView.layer.addSublayer(shapeLayer)
        chartLayers.append(shapeLayer)
        
        for i in 0..<points {
            let angle = angleStep * CGFloat(i) - .pi / 2
            let x = center.x + cos(angle) * maxRadius
            let y = center.y + sin(angle) * maxRadius
            ...
            lineLayer.strokeColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 0.3).cgColor
        ...
        let circlePath = UIBezierPath(ovalIn: CGRect(
            x: center.x - actualRadius,
            y: center.y - actualRadius,
            width: actualRadius * 2,
            height: actualRadius * 2
        ))
        circleLayer.path = circlePath.cgPath
        circleLayer.fillColor = UIColor.clear.cgColor
        circleLayer.strokeColor = UIColor(red: 0.9, green: 0.4, blue: 0.2, alpha: 0.5).cgColor
    ...
    @objc private func resetTapped() {
        ...
        menuButton.setTitle("Select Beam Type ▼", for: .normal)
